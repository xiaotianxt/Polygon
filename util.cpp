...
bool Polygon::in(Point point) // 检查点是否在多边形内部
{
    ...
    if (count >= 2) //极端情况，此时说明该射线正好交在了两条边的交点
    {
        cout << "点不在多边形的内部" << endl;
        return false; // 不在
    }
...
bool CheckCross(Line a, Line b) // 查看两条线是否相交（不规范相交也算
{
    ...
    /***************** 非规范相交 ******************/
    if (equal(p1, 0.0) || equal(p2, 0.0) || equal(p3, 0.0) || equal(p4, 0.0)) // 如果有一个叉乘为0， 说明为非规范相交
    {
        return true;
    }
    /***************** 判断规范相交 ******************/
    else if (p1 * p2 < 0 && p3 * p4 < 0) // 全部异号，说明规范相交
    {
        return true;
    }
    else
